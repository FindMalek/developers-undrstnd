...
import { verifyKey, createApiKey, revokeApiKey } from "@/lib/auth";
    ...
    try {
        ...
        if (!authHeader) {
            return NextResponse.json(
                { error: { message: "Authorization required", type: "authentication_error" } },
                { status: 401 }
            );
        }
        ...
        return NextResponse.json(
            { error: { message: "An error occurred while creating API key", type: "service_error" } },
            { status: 500 }
        );
    }
}
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json(
            { error: { message: "An error occurred while listing API keys", type: "service_error" } },
            { status: 500 }
        );
    }
}
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        if (!keyId) {
            return NextResponse.json(
                { error: { message: "Key ID is required", type: "invalid_request_error" } },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { error: { message: "An error occurred while revoking API key", type: "service_error" } },
            { status: 500 }
        );
    }
} 