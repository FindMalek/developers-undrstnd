...
export async function syncModelsFromProviders() {
    try {
        ...
        if (providers.length === 0) {
            log.warn("MODEL_SYNC_NO_PROVIDERS", { message: "No providers found in database" });
            return;
        }
...
async function syncProviderModels(provider: { id: string; name: string; baseUrl: string; apiKey: string }) {
    try {
        log.info("MODEL_SYNC_PROVIDER_STARTED", { provider: provider.name });

        const response = await axios.get<ProviderModelResponse>(`${provider.baseUrl}/v1/models`, {
            headers: {
...
async function processModelData(
    ...
    try {
        ...
        const existingModel = await database.aIModel.findUnique({
            where: {
        ...
        // Create new model
        const newModel = await database.aIModel.create({
            data: {
                externalId: modelData.id,
                providerId,
                owner: modelData.owned_by,
...
async function notifyNewModel(model: { id: string; providerId: string; externalId: string }) {
    try {
...
export async function linkSiblingModels() {
    ...
    const models = await database.aIModel.findMany({
