...
export async function getApiKeyTierAndLimit(apiKey: string): Promise<{ tier: string; rateLimit: number } | null> {
    try {
        const hashedKey = hashApiKey(apiKey);
...
export async function createApiKey(name: string, tier: "FREE" | "PRO" | "ENTERPRISE", userId?: string) {
