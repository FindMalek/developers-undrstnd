...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      ...
      log.error("MODEL_DISCOVERY_WEBHOOK_UNAUTHORIZED", {
        requestId,
        error: message,
      })
    ...
    if (secret !== process.env.WEBHOOK_SECRET) {
      ...
      log.error("MODEL_DISCOVERY_WEBHOOK_INVALID_SECRET", {
        requestId,
        error: message,
      })
    ...
    if (
      !body.event ||
      body.event !== "model_discovered" ||
      !body.model_id ||
      !body.provider ||
      !body.external_id
    ) {
      const message = parseError(new Error("Invalid payload"))
      log.error("MODEL_DISCOVERY_WEBHOOK_INVALID_PAYLOAD", {
        requestId,
        error: message,
      })
